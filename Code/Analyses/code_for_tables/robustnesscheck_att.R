...
for(j in 2:5){
  ...
  data <- compute_itt_and_att(dataset, j+2, covs = covariates, bwselect='msecomb2')[[3]] %>%
    round(3)
...
knitr::opts_current$set(label = "attresults_robust")
datasummary_df(bind_rows(panel_a, panel_b) %>%
  ...
  kableExtra::save_kable("./Tables/att_mainresults_robust_bw.tex")
